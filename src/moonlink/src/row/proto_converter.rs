...
pub fn proto_to_moonlink_row(p: proto::MoonlinkRow) -> MoonlinkRow {
    MoonlinkRow::new(p.values.into_iter().map(proto_to_row_value).collect())
}
...
fn proto_to_row_value(p: proto::RowValue) -> RowValue {
    use proto::row_value::Kind;
    match p.kind.expect("RowValue.kind is required") {
        ...
        Kind::Array(a) => RowValue::Array(a.values.into_iter().map(proto_to_row_value).collect()),
        Kind::Struct(s) => RowValue::Struct(s.fields.into_iter().map(proto_to_row_value).collect()),
